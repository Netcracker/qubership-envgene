...
**Legacy/unused in this branch** (superseded by the single-job consolidation; directories exist but contain no active code — see their own `CLAUDE.md` for where the functionality moved to):
`base_modules/`, `creds_rotation/` (top-level — real code is at `scripts/creds_rotation/`), `python/integration/`, `python/jschon-sort/`.
...
```
The single-job pipeline scripts under `scripts/` are tested from the repo root with `scripts/` on `PYTHONPATH` (matches CI's `PYTHONPATH=$GITHUB_WORKSPACE:$GITHUB_WORKSPACE/scripts`; see `scripts/CLAUDE.md`):
```bash
