...
public class BomReaderUtilsImplV2 {
    ...
    private void processConfigServiceComponent(Map<String, Map<String, Object>> serviceMap, Component component, String appName, String baseline, Profile override, Bom bomContent) {
        ...
        serviceParams.put("ARTIFACT_DESCRIPTOR_VERSION", new Parameter(checkIfMandatory(bomContent.getMetadata().getComponent().getVersion(), "version in metadata", entity), "sbom", false));
        serviceParams.put("DEPLOYMENT_RESOURCE_NAME", new Parameter(checkIfMandatory(component.getName(), "name", entity) + "-v1", "sbom", false));
        serviceParams.put("DEPLOYMENT_VERSION", new Parameter("v1", "sbom", false));
        serviceParams.put("SERVICE_NAME", new Parameter(checkIfMandatory(component.getName(), "name", entity), "sbom", false));
        if (CollectionUtils.isNotEmpty(component.getComponents())) {
            for (Component subComponent : component.getComponents()) {
    ...
    private Map<String, Object> extractProfileValues(Component dataComponent, String appName, String serviceName,
        ...
        Map<String, Object> profileValues = new TreeMap<>();
        overrideOrigin ="resource-profile-override: " + overrideProfile.getName();
        if (baseline == null) {
            profileService.setOverrideProfiles(appName, serviceName, overrideProfile, profileValues);
            if (MapUtils.isNotEmpty(profileValues)) {
                profileValues = wrapPlainMapWithOrigin(profileValues, overrideOrigin);
            }
        }
        for (ComponentData data : dataComponent.getData()) {
            if (baseline != null && baseline.equals(data.getName().split("\\.")[0])) {
                ...
                if (MapUtils.isNotEmpty(profileValues)) {
                    profileValues = wrapPlainMapWithOrigin(profileValues, baselineOrigin);
                }
                profileService.setOverrideProfiles(appName, serviceName, overrideProfile, profileValues);
                if (MapUtils.isNotEmpty(profileValues)) {
                    profileValues = wrapPlainMapWithOrigin(profileValues, overrideOrigin);
    ...
    private Map<String, Object> wrapPlainMapWithOrigin(Map<String, Object> map, String origin) {
        ...
        for (Map.Entry<String, Object> entry : map.entrySet()) {
            ...
            if (value instanceof Parameter) {
                wrapped.put(entry.getKey(), value);
            } else if (value instanceof Map) {
                wrapped.put(entry.getKey(), wrapPlainMapWithOrigin((Map<String, Object>) value, origin));
            } else if (value instanceof List) {
                wrapped.put(entry.getKey(), wrapPlainListWithOrigin((List<Object>) value, origin));
            } else {
    ...
    private List<Object> wrapPlainListWithOrigin(List<Object> list, String origin) {
        ...
        for (Object item : list) {
            if (item instanceof Parameter) {
                ...
                wrapped.add(wrapPlainMapWithOrigin((Map<String, Object>) item, origin));
            } else if (item instanceof List) {
                wrapped.add(wrapPlainListWithOrigin((List<Object>) item, origin));
            } else {
