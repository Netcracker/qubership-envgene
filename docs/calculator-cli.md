...
| **cluster** | Mandatory | Contains information about the cluster's endpoint. Formed from the `apiUrl`, `apiPort`, `publicUrl`, and `protocol` parameters of the [Cloud](/docs/envgene-objects.md#cloud) object belonging to the Environment Instance for which the Effective Set is being generated | None | [example](#version-20topology-context-cluster-example) |
