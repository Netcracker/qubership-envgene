...
`<service-name>.yaml` Contains parameters for a single service (one file per service). Structure:
...
| **cluster** | Mandatory | Contains information about the cluster's endpoint. Includes `api_url`, `api_port`, `public_url`, and `protocol`. This variable is located in `parameters.yaml` and is formed according to two use cases:<br><br>1. **When a Cloud Passport is used**: The values are taken from the Cloud Passport (either directly or via the cloud object).<br><br>2. **When there is no Cloud Passport**: The values are derived from the `inventory.clusterUrl` attribute in env_definition.yml as follows:<br>- `api_url`: Extracted from the hostname portion of the URL<br>- `api_port`: Extracted from the port portion of the URL<br>- `public_url`: Derived by removing the 'api.' prefix from the hostname<br>- `protocol`: Extracted from the scheme portion of the URL (http/https)<br><br>These values follow the same principles as how the `current_env.cluster.XXX` macros are formed. | None | [example](#version-20topology-context-cluster-example) |
