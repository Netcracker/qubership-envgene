...
Never append `credId` to `remoteRefPath`. Never write `writeToStore`. Always write
`secretStore: default_store` (or a confirmed store id) - EnvGene does not fill omitted values.

## Macro → credRef

Use `scripts/template/replace_macros.py`. Allowed in deploy/e2e ParameterSets only. Forbidden in
`technicalConfigurationParameters`. Built-in fields stay plain `credId` strings.

**Composite values (blocked):** a macro inside a larger string (for example
`user=${creds.get('X').username}@host`) cannot become `$type: credRef`. Stop with `NEEDS_INPUT`;
split into separate parameters before apply.
