...
Default create proposal: `true` - still requires confirmation. Override to omit `create`
(keep the existing value) when the user confirms the value must not be regenerated.

## Script

`draft_credential_template.py` accepts only confirmed decisions (known structure + create +
