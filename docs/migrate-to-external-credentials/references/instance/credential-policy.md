...
[credential-policy.md](../template/credential-policy.md) for Template-owned
...
`writeToStore` is migration-plan only (transfer passwords and tokens to Store). Never add it to final YAML.
Skills/scripts in this folder do not perform Store I/O - they only record the operator choice.

One Secret Store per repository (`default_store` in `configuration/secret-stores.yml`). Credential
entries must set `secretStore` explicitly (usually `default_store`). JSON Schema documents a
default, but the Effective Set calculator reads `Credential.secretStore` as-is with no runtime
fallback.

Deployer credentials (`app-deployer/deployer-creds.yml`) are out of scope for No-CMDB - delete,
do not convert.
...
| Cluster-level Shared (`environments/<cluster>/credentials/` or legacy `shared-credentials/`) | `external-tier` | `shared` |
...
## Classification review signals

Heuristic markers route to `needsReview: true` - they do not auto-apply tier or path:

- cred-id matches platform patterns: `dbaas`, `argocd`, `arango`, `consul`, `cluster`, `keycloak`,
  `vault`, `k8s`, and similar
- cred-id contains: `cluster`, `admin`, `dba`, `root`, `bootstrap`, `master`
- YAML comment mentions: `cloud passport`, `platform`, `script generated`, `shared across envs`

When confirmed cluster-shared: prefer passport-tier shape (`remoteRefPath: <cluster>`, omit
`create`). Show evidence and proposals; never auto-apply without confirmation.
...
- Orphaned file (no referenced cred-ids): delete, do not convert.

### System Credentials

- `tier: external-tier`; create always `false` / omit; path approved system path (fallback
  `external`); `writeToStore: true` before removing `data`; Vault/GCP only per How-to.
- `app-deployer/deployer-creds.yml`: out of scope (No-CMDB) - delete.
...
- No `writeToStore` in final YAML. Always set `secretStore` on credential entries. No secret values in
  inventory/report.
