...
def main() -> None:
    ...
    for desc in descriptors:
        ...
        files = list_credential_scan_files(
            repo, desc, include_credential_template=True
        )
        merged: dict = {}
        for fpath in files:
            try:
                ...
                decisions.append(
                    {
                        ...
                        "credId": cid,
                        "message": (
                            "Confirm create, remoteRefPath before draft. "
        ...
        [
            {
                "id": "secret_store",
                "status": "NEEDS_INPUT",
                "message": "Confirm Secret Store id to write on each entry (default_store).",
            },
        ]
    )
