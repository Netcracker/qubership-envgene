...
def main() -> None:
    ...
    kinds = {b["kind"] for b in data["blockers"]}
    assert "shared_ref_has_extension" in kinds, data["blockers"]
    assert "undeclared_cred_id" in kinds, data["blockers"]
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Fix known preflight blockers so convert path can proceed
        env_def = work / "environments/cluster/env/Inventory/env_definition.yml"
        env_def.write_text(
            env_def.read_text(encoding="utf-8").replace(
                "- shared-credentials.yml", "- shared-credentials"
            ),
            encoding="utf-8",
        )
        # Declare test-cred used by hash macro in passport main
        passport = work / "environments/cluster/cloud-passport/cluster-creds.yml"
        passport.write_text(
        ...
        proc = run(["preflight.py", "--repo", str(work)])
        data = parse(proc)
        assert proc.returncode == 0, proc.stdout
        assert data["status"] == "ok", data.get("blockers")

        passport_rel = "environments/cluster/cloud-passport/cluster-creds.yml"
        ...
        assert written["shared-cred"]["secretStore"] == "default_store"
