...
The Effective Set is written to `environments/<cluster>/<env>/effective-set/` in the Instance repository by default.

To publish to an external GitLab repository instead (or in addition), set `effective_set.placement` in
`env_definition.yml` and follow [Publish Effective Set to an external GitLab repository](#publish-effective-set-to-an-external-gitlab-repository).
For behaviour and validation rules, see
...
For external publish, you also set `effective_set.placement` in this file. See
[Publish Effective Set to an external GitLab repository](#publish-effective-set-to-an-external-gitlab-repository).
...
The `git_commit` job commits these files to the Instance Repository automatically. The pipeline run is complete when
`git_commit` succeeds and the files appear under `environments/prod-cluster/prod-01/effective-set/`.

When `effective_set.placement` is `remote` or `dual`, the Instance path may not update in the same way. See
[Publish Effective Set to an external GitLab repository](#publish-effective-set-to-an-external-gitlab-repository).
...
|------------------------------|---------------|
| `endpoint_unreachable`       | GitLab URL reachable from the pipeline runner. Matches `ARGOCD_GITLAB_URL` or `DCL_GIT_URL`. |
| `authentication_failed`      | GitLab token valid. Token can push to the publish branch. |
