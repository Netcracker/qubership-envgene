...
Work through Steps 1-6 for one Environment Template, then repeat for the next. For each
...
This how-to only cuts over the Template YAML. It does not move any actual password or token value -
that is a separate step, see [Transfer Credential Values to the Secret
Store](transfer-credential-values-to-external-credentials.md), run between this guide and
...
If you transfer an existing Git value into the Store during migration, that transfer is a
separate action - see [Transfer Credential Values to the Secret
Store](transfer-credential-values-to-external-credentials.md). That transfer never appears in the
...
(`bss.yaml` descriptor -> `bss.yml.j2` Credential Template):
...
In the Template Descriptor for this Environment Template, add `external_credential_template`. The
path stem must match the Credential Template filename stem from Step 3.
...
If actual passwords or tokens for the Credentials you just templated still live in the Instance
Repository as Git-committed values, collect them now, before Instance YAML cutover converts those
entries to `type: external` - see [Transfer Credential Values to the Secret
Store](transfer-credential-values-to-external-credentials.md), Step 2 (`migration-cli collect`).
Credentials with `create: true` do not need this - EnvGene generates a new value for them.
