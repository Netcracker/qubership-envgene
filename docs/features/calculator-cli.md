...
- [Calculator CLI](#calculator-cli)
  ...
  - [Proposed Approach](#proposed-approach)
    ...
    - [Effective Set v2.0](#effective-set-v20)
      ...
      - [\[Version 2.0\] Traceability in the Effective Set](#version-20-traceability-in-the-effective-set)
...
| `--enable-traceability`/`-etr`                      | boolean | no        | Determines whether [traceability](#version-20-трейсебилити-в-эффектив-сете) will be enabled. If `true`, traceability comments will be added. If `false`, they will be omitted.                                                | `true`  | `false`                                                                               |
...
#### [Version 2.0] Traceability in the Effective Set
...
| Calculator `--extra_params`                        | `# envgene pipeline parameter`             | `DEPLOYMENT_SESSION_ID: "7e9f5f54-4be2-4fbd-a267-19e78d09810d" # envgene calculated`  |
...
5. The source is determined by priority: if a parameter is defined in multiple sources, the source with the highest priority is used.
...
9. If all child keys/items inside an object/list have the same source, put the comment only on the parent key instead:

    ```yaml
    # Different sources — comments on each element
    servers:
    ...
    # Same source — comment on the parent
    servers: # cloud
      - name: "server1"
        host: "host1"
        port: 8080
