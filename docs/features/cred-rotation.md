...
8. `CRED_ROTATION_PAYLOAD` and `GET_PASSPORT: true` cannot be set in the same pipeline run. The pipeline fails at validation before any job starts if both are set.

### Requirements

1. Credential rotation operation is performed in a separate `credential_rotation` job
   1. The job runs before `bg_manage` and before all other per-environment jobs
