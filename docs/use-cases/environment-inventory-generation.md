...
1. The `env_inventory_generation` job runs:
   ...
   3. Resolves target path by `place`:
      ...
      - `place=cluster` → `/environments/<cluster-name>/configuration/<name>.yml`
      - `place=site` → `/environments/configuration/<name>.yml`
   4. Creates the Shared Template Variable file using `content` (create-or-replace semantics; in this UC the file is expected to be missing).
...
1. The `env_inventory_generation` job runs:
   ...
   4. Applies some operations (examples of partial progress):
      ...
      - Creates or replaces files (e.g., `env_definition.yml`, paramset files, credential files, resource profile overrides).
