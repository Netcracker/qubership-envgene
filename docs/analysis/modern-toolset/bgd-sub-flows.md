...
- [BGD sub-flows](#bgd-sub-flows)
  ...
  - [Sub-flow 2 - BGD_WARMUP](#sub-flow-2---bgd-warmup)
...
| `OPERATION_TYPE`       | `BGD_INIT`, `BGD_WARMUP`, `BGD_PROMOTE`, `BGD_ROLLBACK`, `BGD_COMMIT`, `DEPLOY` | selects the sub-flow                 |
...
Applies to `BGD_INIT`, `BGD_PROMOTE`, `BGD_ROLLBACK`, and `BGD_COMMIT`. Pure state change with no new deploy.

Flow:

```text
1.1 preprocess -> 1.4 change_bg_state -> 1.15 generate_effective_set -> 1.18 postprocess
              -> 1.19 git_commit -> 1.20 es_pusher
```
...
OPERATION_TYPE: BGD_INIT        # or BGD_PROMOTE, BGD_ROLLBACK, BGD_COMMIT
...
## Sub-flow 2 - BGD_WARMUP

`BGD_WARMUP` replicates the active namespace into the candidate.

Flow:

```text
1.1 preprocess -> 1.4 change_bg_state -> 1.5 warmup -> 1.13 generate_deployment_plan -> 1.15 generate_effective_set
              -> 1.16 generate_argocd_repo -> 1.18 postprocess -> 1.19 git_commit -> 1.20 es_pusher
              -> [job 2] sync
```
...
resolved upstream (bg-controller API) to the physical `ORIGIN`/`PEER` side. It is not a `BGD_*` operation, it is
...
1.1 preprocess -> (1.2 get_passport, 1.3 credential_rotation, 1.6 inventory, 1.8 set_template_version - if requested)
              ...
              -> 1.16 generate_argocd_repo -> 1.18 postprocess -> 1.19 git_commit -> 1.20 es_pusher -> [job 2] sync
```
