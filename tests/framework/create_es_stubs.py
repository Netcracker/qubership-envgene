...
FEATURE_DIR = Path('/workspace/tests/features/effective_set_generation')
...
def process_features():
    for feature_file in FEATURE_DIR.glob('*.feature'):
        ...
        for scenario in scenarios[1:]:
            ...
            # Extract UC code like UC-ES-TOP-1
            match = re.match(r'(UC-[A-Z0-9\-]+):', title)
            if match:
                uc_code = match.group(1).lower().replace('-', '_')
                ref_name = match.group(1).lower()
            else:
                # Fallback if no UC code
                safe_title = re.sub(r'[^a-zA-Z0-9]', '_', title).lower()[:15]
                uc_code = f"es_gen_{safe_title}"
                ref_name = f"es-gen-{safe_title.replace('_', '-')}"
            ...
            # Minimal effective set in golden
            golden_env_dir = golden_dir / 'effective_set'
            golden_env_dir.mkdir(parents=True, exist_ok=True)
            (golden_dir / 'Inventory' / 'env_definition.yml').parent.mkdir(parents=True, exist_ok=True)
...
if __name__ == '__main__':
    process_features()
    print("Done generating stubs for effective_set_generation.")
