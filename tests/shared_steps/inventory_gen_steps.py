...
def file_is_created(workspace, filename):
    ...
    assert (env_dir / "Inventory" / filename).exists(), (
...
def no_inventory_files_modified(workspace):
    env_dir = workspace.builder.get_env_dir("test-cluster", "test-env")
    assert not (env_dir / "Inventory" / "env_definition.yml").exists(), (
        "Inventory should not have been created"
    )
...
@given(
    parsers.parse(
        'the target specific template version file "{name}" does not exist at "{scope}" scope'
    )
)
...
@given(
    parsers.parse(
        'the target specific template version file "{name}" exists at "{scope}" scope'
    )
)
...
@when(
    parsers.parse(
        'the Instance pipeline is started with ENV_INVENTORY_CONTENT specifying "{action}" for specific_template_version "{name}" at "{scope}" scope'
    )
)
...
@then(
    parsers.parse(
        'the specific template version file "{filename}" is created at "{scope}" scope'
    )
)
@then(
    parsers.parse(
        'the specific template version file "{filename}" is updated at "{scope}" scope'
    )
)
...
@then(
    parsers.parse(
        'the specific template version file "{filename}" is deleted at "{scope}" scope'
    )
)
...
@given(
    parsers.parse(
        'the target template variable file "{name}" does not exist at "{scope}" scope'
    )
)
...
@when(
    parsers.parse(
        'the Instance pipeline is started with ENV_INVENTORY_CONTENT specifying "{action}" for template_variable "{name}" at "{scope}" scope'
    )
)
...
@then(
    parsers.parse(
        'the template variable file "{filename}" is created at "{scope}" scope'
    )
)
