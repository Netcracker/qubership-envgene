...
""")
    ...
    assert [event["type"] for event in events] == ["start", "running", "running", "running", "running", "stop"]
    assert len({event["traceid"] for event in events}) == 1
    assert events[-1]["status"] == "SUCCESS"
    assert events[-1]["data"]["envgeneVersion"] == "1.2.3"
    assert events[3]["data"]["steps"] == [{"name": "env_build", "status": "SUCCESS", "durationMs": 10}]
