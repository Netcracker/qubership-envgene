...
class MetricsCollectorActivity:
    ...
    def send_running(self, status: str = "IN_PROGRESS", results: list[Any] | None = None) -> None:
        self._send("running", status, results)

    def send_stop(self, status: str) -> None:
        self._send("stop", status)
