...
from envgenehelper import get_cluster_name_from_full_name, dumpYamlToStr, get_environment_name_from_full_name, readYaml, cleanup_dir
...
class TestEnvInvGen(BaseTest):
    ...
    def test_generate_env_create(self):
        ...
        content = readYaml(text=template.render(places=places, action=action, env=env_name, cluster=cluster), safe_load=True)
        ...
        for item in content.get("paramSets", []):
            self._assert_item(env_dir, item, "parameters", INVENTORY)

        for item in content.get("credentials", []):
            self._assert_item(env_dir, item, "credentials", INVENTORY)

        for item in content.get("resourceProfiles", []):
            self._assert_item(env_dir, item, "resource_profiles", INVENTORY)

        for item in content.get("sharedTemplateVariables", []):
            self._assert_item(env_dir, item, "shared-template-variables")

    def _assert_item(self, env_dir, item, subdir, inventory=""):
