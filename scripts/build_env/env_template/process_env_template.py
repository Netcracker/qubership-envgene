...
from envgenehelper import get_env_definition, fetch_cred_value
...
def download_artifact_new_logic(env_definition: dict) -> str:
    ...
    if dd_artifact_info:
        ...
        logger.info(f"Parsed maven coordinates: group_id={group_id}, artifact_id={artifact_id}, version={version} from dd")
...
def download_artifact_old_logic(env_definition: dict, project_dir: str) -> str:
    ...
    if dd_repo_url:
        dd_url = artifact.create_artifact_url_by_parts(dd_repo_url, group_id, artifact_id, version, FileExtension.JSON)
