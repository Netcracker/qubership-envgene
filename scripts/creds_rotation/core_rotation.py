...
def process_entry_in_payload(
    ...
    if entry.application and entry.context == "pipeline":
        raise ValidationError(ErrorMessages.INVALID_CONTEXT.format(param_key=entry.parameter_key),
    ...
    if not target:
        raise ReferenceError(ErrorMessages.ENTITY_FILE_NOT_FOUND.format(param_key=entry.parameter_key),
    ...
    if param_type not in target_yaml:
        raise ValidationError(ErrorMessages.MISSING_CONTEXT.format(context=entry.context, target_file=target_file,
    ...
    if cred_field not in {"username", "password", "secret"}:
        raise ValidationError(ErrorMessages.INVALID_CRED_FIELD.format(cred_field=entry.credential_field),
