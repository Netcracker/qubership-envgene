# creds_rotation — Credential Rotation Step

Given a JSON payload (`CRED_ROTATION_PAYLOAD`) describing which parameters to rotate and their new values, it locates the affected credential and instance files, updates them, and writes an `affected-sensitive-parameters.yaml` report. Runs as `CredentialRotationStep` in `scripts/pipeline/orchestrator.py` — not a standalone Docker image (see `scripts/CLAUDE.md`).
