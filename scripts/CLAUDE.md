...
| `cloud_passport/` | `main.py` (`run_cloud_passport` — discovery download via `GitLabClient`), `cloud_passport.py` (`process_cloud_definition`, `add_cloud_passport_creds`, `mergeDeployParametersFromPassport`), `cmdb.py` |
| `creds_rotation/` | Credential rotation step — see `creds_rotation/CLAUDE.md` (repo root) |
...
```
`scripts/tests/conftest.py` overrides `business_helper.get_schema_dir` to point at the repo's `schemas/` directory.
