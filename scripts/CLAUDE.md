...
|-----------|-----------------|
| `pipeline/` | `orchestrator.py` (steps above), `pipeline_parameters.py` (`PipelineParametersHandler.from_env(full_env_name)` — writes `envgene-vars.env`), `resolve_env_names.py` (validates env selection, writes `envgene-resolved.env`), `pipeline_manager.py` |
