...
def assert_namespaces_same_content(
    ...
    """Assert that two namespace directories have identical manifest.yaml content.

    Used by Warmup (UC-BG-2) and Reverse Warmup (UC-BG-6) to verify the copy
    of the active namespace folder into the candidate namespace folder.
    """
    env_dir = workspace.builder.get_env_dir(workspace.cluster_name, workspace.env_name)
    ns_dir = env_dir / "Namespaces"

    first_manifest = ns_dir / first_ns / "manifest.yaml"
    second_manifest = ns_dir / second_ns / "manifest.yaml"

    assert first_manifest.exists(), (
        f"manifest.yaml missing in namespace {first_ns!r}: {ns_dir / first_ns}"
    )
    assert second_manifest.exists(), (
        f"manifest.yaml missing in namespace {second_ns!r}: {ns_dir / second_ns}"
    )

    first_content = first_manifest.read_text(encoding="utf-8")
    second_content = second_manifest.read_text(encoding="utf-8")

    assert first_content == second_content, (
        f"Namespace content mismatch between {first_ns!r} and {second_ns!r}.\n"
