...
public class BomReaderUtilsImplV2 {
    ...
    private void addImageParameters(Component component, Map<String, Object> deployParams, Map<String, Object> collidingImageDeployParams, Set<String> serviceNames) {
        if (component.getMimeType().equalsIgnoreCase(APPLICATION_OCTET_STREAM)) {
            ...
            if (StringUtils.isNotEmpty(key)) {
                ...
                if (serviceNames.contains(key)) {
                    collidingImageDeployParams.put(key, new Parameter(value, SBOM_ORIGIN, false));
                } else {
                    deployParams.put(key, new Parameter(value, SBOM_ORIGIN, false));
                }
            }
        }
    }
