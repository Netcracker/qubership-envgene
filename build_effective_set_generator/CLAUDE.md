...
Pure Java/Maven multi-module project (Quarkus). Builds the Calculator CLI that resolves parameter macros (jinjava/groovy) and merges layered parameters into the final Effective Set. **No Python in this directory** in this branch — the Python orchestration layer that calls this CLI lives under `scripts/effective_set/` and `scripts/utils/` at the repo root (see `scripts/CLAUDE.md`).
