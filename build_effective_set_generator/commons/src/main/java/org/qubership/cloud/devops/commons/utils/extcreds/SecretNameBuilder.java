...
public class SecretNameBuilder {
    ...
    public static String buildNormalizedSecretName(String remoteRefPath, String credId, SecretStoreType secretStoreType) {
        if (isNullOrBlank(remoteRefPath) || isNullOrBlank(credId) || secretStoreType == null) {
            throw new IllegalArgumentException
                    (String.format("Inputs to build Normalized secret name is null. remoteRefPath = %s, credId = %s, secretStoreType = %s", remoteRefPath, credId, secretStoreType));
        }
    ...
    private static void validate(String input, Pattern pattern, String type) {
        if (!pattern.matcher(input).matches()) {
            throw new IllegalArgumentException(String.format("Invalid characters in %s [%s] for type %s. %n Allowed pattern: %s. %n Please validate cred id and remoteRefPath.", SECRET_NAME, input, type, pattern));
        }
    }
    ...
    private static void validateLength(String input, int max, String fieldLabel, String type) {
        ...
        if (length > max) {
            throw new IllegalArgumentException(String.format("Length exceeded for %s [%s] for type %s. Max allowed: %d, actual: %d", fieldLabel, input, type, max, length));
        }
    }
