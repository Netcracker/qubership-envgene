# creds_rotation — Moved to `scripts/creds_rotation/`

This top-level directory contains only this file. In this branch, credential rotation is a **step in the single-job pipeline** (`CredentialRotationStep` in `scripts/pipeline/orchestrator.py`), not a standalone Docker image. The real code lives at `scripts/creds_rotation/`.

Given a JSON payload (`CRED_ROTATION_PAYLOAD`) describing which parameters to rotate and their new values, it locates the affected credential and instance files, updates them, and writes an `affected-sensitive-parameters.yaml` report.

## Scripts (`scripts/creds_rotation/`)

| File | Responsibility |
|------|---------------|
| `creds_rotation_handler.py` | Entry point `run_cred_rotation()`: validates env vars, reads all files, iterates payload entries via `process_entry_in_payload`, optionally writes updates |
...
| `models.py` | Dataclasses: `PayloadEntry`, `RotationResult`, `AffectedParameter`, `ParameterReference`, `FileUpdateEntry`, `CredMap`, `EnvConfig` |
...
```bash
PYTHONPATH=".:./scripts" python -m pytest scripts/tests/creds_rotation/
```
Test file: `scripts/tests/creds_rotation/test_creds_rotation_handler.py`.
