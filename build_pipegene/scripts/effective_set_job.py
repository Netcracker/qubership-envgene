...
def prepare_generate_effective_set_job(pipeline, full_env_name, env_name, cluster_name, app_reg_defs_job,
    ...
    logger.info(f'Prepare process_sd job for {full_env_name}')
    ...
    if full_sd_exists or sd_data:
        cmdb_cli_cmd_call.append("--registries=$CI_PROJECT_DIR/configuration/registry.yml")
        cmdb_cli_cmd_call.append(f"--sboms-path={sboms_path}")
        cmdb_cli_cmd_call.append(f"--sd-path={sd_path}")
