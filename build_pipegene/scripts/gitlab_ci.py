...
from pipeline_helper import get_gav_coordinates_from_build, find_predecessor_job, get_env_artifact_paths, do_checkout
...
def build_pipeline(params: dict, sensitive_params: list) -> None:
    ...
    for full_env_name in env_names:
        ...
        env_artifact_paths = get_env_artifact_paths(cluster_name, environment_name)
        ...
        if params['GET_PASSPORT'] and cluster_name not in get_passport_jobs:
            ...
            jobs_map["get_passport_job"] = prepare_passport_job(pipeline, full_env_name, environment_name, 
                                                                cluster_name, env_artifact_paths)
        ...
        if not params.get('BG_MANAGE', None):
            ...
            jobs_map['bg_manage_job'] = prepare_bg_manage_job(pipeline, full_env_name, env_artifact_paths)
        ...
        if (any(job in jobs_map for job in plugin_params['jobs_requiring_git_commit']) and
            ...
            jobs_map["git_commit_job"] = prepare_git_commit_job(pipeline, full_env_name, environment_name, cluster_name,
                                                                env_artifact_paths, credential_rotation_job)
        else:
