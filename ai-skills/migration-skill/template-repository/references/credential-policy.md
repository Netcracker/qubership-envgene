...
Do **not** use `{{ current_env.namespace }}` unless implementation availability is proven.
...
- Jinja path uses only available variables (`cloud`, `name` - not unproven `namespace`).
