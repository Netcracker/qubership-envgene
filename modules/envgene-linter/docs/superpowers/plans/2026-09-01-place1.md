...
| `testdata/golden/instance/…` | Tiny repo for Effective Set |
...
Blank line between findings. No findings:
...
Golden is a tiny instance repo (no template) plus expected leaves written by applying the documented algorithm by hand. Do not copy a generated `Namespaces/` file that includes template sources — that would disagree with this cycle’s Effective Set.
...
Deviation from spec wording: golden is a hand-derived expected-leaves file for a tiny no-template repo, not a copied generated `Namespaces/*.yml` (those include template sources and would be a false oracle this cycle).
