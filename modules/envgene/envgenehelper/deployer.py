...
def get_cred_file_path(deployer_dir):
    ...
    if check_file_exists(dashes_cred_path):
        ...
        logger.error(
            f"Credentials definition for deployer not found in either {dashes_cred_path} or {folder_cred_path}")
...
def get_deployer(env_name, instances_dir):
    ...
    if "deployer" not in data["inventory"]:
        logger.warning("'deployer' key is not defined in inventory section of env definition")
        return None
...
def get_deployer_config():
    ...
    instances_dir = base_dir + '/environments'
    deployer_name = get_deployer(full_env_name, instances_dir)
    if not deployer_name:
        return "", "", ""
    env_deployer_file_path = find_env_deployer_definition(full_env_name, instances_dir)
    ...
    if deployer_name not in data:
        logger.info(
            f"Deployer with key {deployer_name} not found in environment specific configuration for {full_env_name}. Going to root configuration: {basic_deployer_file_path}")
