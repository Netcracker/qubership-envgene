...
| `git_helper.py` | `GitRepoManager` — configure/stage/detached-commit/cherry-pick+push/sparse-checkout via GitPython; `GitContext` (platform detection from env); `GitLabClient` (pipeline bridges/jobs, artifact download, project variables) |
| `repo_paths.py` | `REPO_ROOT_PATHS`, `get_env_artifact_paths()`, `get_shared_entity_paths()` — path whitelists used by `GitRepoManager.get_sparse_checkout_paths` / `stage_changes` |
