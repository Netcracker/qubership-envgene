...
The console groups findings by rule. It always prints all 13 rule headings; a rule without findings shows `No findings`.
...
Reports can contain configuration values in finding messages. The tool does not upload reports; inspect their contents before sharing them or publishing them as CI artifacts.
...
- Missing references do not produce a dedicated missing-file finding.
...
- Application and Registry Definitions are not checked merely because their directories exist; current local inputs do not establish their runtime usage.
