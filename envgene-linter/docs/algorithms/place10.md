...
Catalog description: `Entities belong in their type directories`. The console always prints thirteen rule headers; PLACE-10 follows PLACE-9 and precedes NAME-1. A silent block shows `No findings`. HTML retains only sections with findings. Findings preserve CLI exit code `0`; no new flags or automatic edits are introduced.
